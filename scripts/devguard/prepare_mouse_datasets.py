...
from devguard.fixtures import create_quick_fixture, create_stress_fixture
...
def prepare_stress_dataset(config: dict, output_path: str | Path | None = None) -> Path:
    registry = dataset_registry_frame(config)
    row = registry.loc[registry["dataset_id"].eq("devguard_stress_mouse")].iloc[0]
    output = Path(output_path or row["local_path"])
    metadata_output = Path(row["metadata_output"])
    adata = create_stress_fixture()
    adata = standardize_obs(adata, dataset_id="devguard_stress_mouse")
    missing = validate_obs_schema(adata.obs)
    if missing:
        raise ValueError(f"Stress fixture missing DevGuard obs columns: {missing}")
    write_h5ad(adata, output)
    write_dataframe(metadata_summary(adata.obs), metadata_output)
    write_manifest(
        "results/devguard/dataset_metadata/devguard_stress_mouse_manifest.json",
        name="prepare_stress_dataset",
        inputs=["generated_stress_fixture"],
        outputs=[str(output), str(metadata_output)],
        parameters={"software_validation_only": True, "contains_noise_batch_donor_dropout_label_noise": True},
        metrics={"n_cells": int(adata.n_obs), "n_genes": int(adata.n_vars)},
    )
    return output


def prepare_registry(config_path: str | Path, quick_fixture: bool = False, stress_fixture: bool = False) -> None:
...
def main() -> None:
    ...
    parser.add_argument("--stress-fixture", action="store_true", help="Generate the noisy software validation fixture.")
    args = parser.parse_args()
    prepare_registry(args.config, quick_fixture=args.quick_fixture, stress_fixture=args.stress_fixture)
