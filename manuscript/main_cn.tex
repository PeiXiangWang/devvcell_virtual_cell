...
\subsection*{MouseGastrulationData integrated chimera perturbation analysis}

Bioconductor \texttt{MouseGastrulationData} 已安装，并已全量导出 WT chimera、Tal1 chimera 和 T chimera 数据。主分析使用 integrated chimera controls：WT control cells 加 Tal1/T chimera tomato-negative host controls。为降低数据源偏移，SVD embedding 后按 source dataset 做 latent centering，再建立 E8.5 strict sample-level normality reference。
...
\midrule
within\_stage\_normal & 19,061 & 0.9548 \\
fate\_deviation & 675 & 0.0338 \\
abnormal\_off\_normal & 228 & 0.0114 \\
\bottomrule
\end{tabular}
\end{center}
...
class & cells & fraction \\
\midrule
within\_stage\_normal & 16,297 & 0.5758 \\
fate\_deviation & 7,415 & 0.2620 \\
abnormal\_off\_normal & 4,593 & 0.1623 \\
\bottomrule
\end{tabular}
\end{center}

Compared with heldout controls, Tal1 has strong enrichment of \texttt{abnormal\_off\_normal} cells (odds ratio 16.77, FDR reported as 0 by numerical underflow) and \texttt{fate\_deviation} cells (odds ratio 10.14, FDR reported as 0).
...
class & cells & fraction \\
\midrule
within\_stage\_normal & 14,922 & 0.9434 \\
fate\_deviation & 674 & 0.0426 \\
abnormal\_off\_normal & 221 & 0.0140 \\
\bottomrule
\end{tabular}
\end{center}

T shows only modest enrichment over heldout controls: \texttt{abnormal\_off\_normal} odds ratio 1.23 and \texttt{fate\_deviation} odds ratio 1.27. Thus DevGuard separates a strong Tal1 abnormality/deviation phenotype from a largely tolerated T/Brachyury chimera state at E8.5.
...
\begin{enumerate}
  ...
  \item 对 high-FPR lineages 做 sensitivity analysis 或从主 lineage-level 解释中降权；
  \item 继续追加更多公开小鼠 genetic/chemical perturbation datasets，扩大扰动类型覆盖。
\end{enumerate}
