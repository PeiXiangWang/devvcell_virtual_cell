...
\subsection*{GSE212050 control reference}

GSE212050 已下载 Seurat RDS，导出 matrix/obs/var，并构建 15,000-cell downsample H5AD。
...
平均 heldout FPR 接近 \(\alpha=0.05\)。最大 FPR 来自低 heldout 组，例如 \texttt{d4\_d4.5 / Intermediate mesoderm} 只有 3 个 heldout cells，已用 \texttt{low\_heldout\_flag} 标出。正式结果需要完整数据或更大 downsample，并过滤低 sample-unit group。
